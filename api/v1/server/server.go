...
type Server struct {
	...
	SocketPath    string `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/cilium-api.sock"`
	...
	TLSCertificate    string `long:"tls-certificate" description:"the certificate to use for secure connections" env:"TLS_CERTIFICATE"`
	TLSCertificateKey string `long:"tls-key" description:"the private key to use for secure connections" env:"TLS_PRIVATE_KEY"`
	TLSCACertificate  string `long:"tls-ca" description:"the certificate authority file to be used with mutual tls auth" env:"TLS_CA_CERTIFICATE"`
...
func (s *Server) Start(cell.HookContext) (err error) {
	// configure API and its handlers
	s.ConfigureAPI()
...
func (s *Server) Listen() error {
	...
	if s.hasScheme(schemeUnix) {
		addr, err := net.ResolveUnixAddr("unix", string(s.SocketPath))
		if err != nil {
			return err
		}
		domainSocketLenter, err := net.ListenUnix("unix", addr)
		if err != nil {
			return err
		}
		s.domainSocketL = domainSocketLenter
	}
